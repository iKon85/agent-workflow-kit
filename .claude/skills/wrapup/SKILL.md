...
## Ausführungs-Modell — 3 Phasen (Empfehlung: mechanischer Teil auf Sonnet)

**Warum empfohlen:** `model:`-Frontmatter überschreibt das Session-Modell laut Doku **nur „für den Rest des aktuellen Turns" — beim nächsten Prompt greift wieder das Session-Modell** ([skills.md](https://code.claude.com/docs/en/skills.md)). `wrapup` ist **mehr-Turn-interaktiv** (Retro-Gate, Annahme-Drift-Bestätigung): jede User-Gate-Antwort startet einen neuen Turn → das Frontmatter fiele nach dem ersten Gate zurück, der **mechanische Großteil läuft danach** also wieder auf dem Session-Modell (in durchweg Opus belegt). Darum trägt der Skill **kein** `model:`-Frontmatter mehr; **empfohlen** ist stattdessen, die mechanische git/gh-Plumbing an einen **Sonnet-Subagent** zu geben (der läuft ganz auf seinem eigenen Modell, turn-unabhängig). Alle User-Gates **und** die Secret-Review bleiben im **Hauptthread** (= Session-Modell, das <maintainer> gewählt hat — Sonnet *oder* Opus; der Subagent erzwingt Sonnet für den Bulk unabhängig davon).

**Bedingung — kein Hard-Contract:** Dieser 3-Phasen-Schnitt lohnt sich nur, **wenn dein Harness echten Subagent-Dispatch mit Model-Param kann** (hier: `Agent`-Tool + `model: sonnet`). **Kann dein Harness das nicht** (kein Subagent-Dispatch, kein Model-Override pro Agent) → **läuft alles inline im Hauptthread**: Phase 1+2+3 in einem Lauf, gleiche Steps, gleiche Reihenfolge, gleiche Gates — nur ohne Phasenwechsel. Die **STOP-zurück-Regel bleibt davon unberührt und gilt unverändert scharf** (s. u.): bei jedem Hard-Stop abbrechen, NICHTS forcen, den Grund explizit melden — inline heißt das: der Hauptthread hält an derselben Stelle an, statt weiterzumachen.
...
**Dispatch-Contract (Phase 1 → Phase 2, nur falls dein Harness Subagent-Dispatch mit Model-Param kann):** Wenn Phase 1 grün durch ist (alle Gates beantwortet, Commit liegt lokal), **einen** Subagent dispatchen — `Agent`-Tool, **`model: sonnet`** explizit (kein `CLAUDE_CODE_SUBAGENT_MODEL`-Env-Pin gesetzt — sonst schlüge er den Param still, Gotcha; bei Unsicherheit `printenv | grep SUBAGENT`). Prompt übergibt **alle** in Phase 1 gesammelten Werte:
...
**STOP-zurück-Regel (nicht verhandelbar):** Trifft der Subagent **irgendeinen** Hard-Stop (Push abgelehnt · `pr-body-check` Exit 1 · Merge-Gate `CONFLICTING`/roter Check · Merge nicht `MERGED` · `worktree remove`/`branch -d` refuse't · **jeder Secret-Grep-Treffer** im — eigentlich schon in Phase 1 committeten — Diff) → **abbrechen, NICHTS forcen, an den Hauptthread zurückmelden** (Grund + Stelle). Der Subagent **klärt keine Security-Judgment-Frage selbst** und ruft **kein `AskUserQuestion`** (kann er nicht) — er meldet, der Hauptthread entscheidet.
...
> Die folgenden Steps sind je mit `[Phase N]` getaggt — das ist die **empfohlene** Zuordnung bei Subagent-Dispatch. Phase 1 + 3 führt der Hauptthread aus; **alles mit `[Phase 2]` läuft im Sonnet-Subagent, sofern dein Harness das kann** — kann er es nicht, führt der Hauptthread diese Steps **selbst inline aus**, in derselben Reihenfolge, ohne die Gates/Regeln zu lockern.
<!-- mirror-xform:end -->
...
<!-- mirror-xform:start codex-wrapup-phase-labels -->
1. **Anker-Tracker-Sync — `anchor-sync`** `[Phase 2 · Sonnet-Subagent]`. Statt Hand-Tick: `python3 scripts/board-sync.py anchor-sync <anker#> --dry-run` → Diff sichten → dann ohne `--dry-run` schreiben. Das regeneriert die **volatilen Spalten der Slices-Tabelle aus dem Board** — **Status** (`✅ #<PR>` merged · `🔄` offener PR/In-Arbeit · `⬜` sonst) und **Branch** (tatsächlicher PR-`headRefName`) — **monoton** (kippt nie ein vorhandenes `✅`/`🔄` zurück, falls das Sub-Issue als `Part of`-gen-a/gen-b offen bleibt und keinen Closing-PR trägt) und **driftfrei idempotent** (ein No-Drift-Lauf reproduziert die Tabelle byte-genau). **Stabile Plan-Spalten** (Slice/Modell/Gate/blocked-by) bleiben **verbatim**; Hand-Annotationen wie `✅ (gen-a)` überleben. **Fehlende Sub-Issue-Zeilen** (mid-wave entstandene Splits, der Gen-b-Fall) hängt es als Frischzeilen an — die Ausgabe nennt sie (`+N new sub-issue row(s)`); deren **stabile Zellen (Slice/Gate/Modell) danach von Hand füllen**. **Was `anchor-sync` NICHT managt:** das **Gate-Symbol** (nicht board-getrackt) → bei Bedarf separat von Hand korrigieren. Die Status-Spalte ist damit **board-abgeleitet**, nicht mehr Hand-gepflegt; der native „Sub-issues progress"-Rollup ist die %-Zweitsicht. Ersetzt den alten Hand-Tick + Step-6-Reminder (Retro: Gen-b fehlte + stale Branch `-drop` — `anchor-sync` fängt beides). Skript-Fehler / kein Slice-Table im Anker → **stop + vorschlagen**.
<!-- mirror-xform:end -->

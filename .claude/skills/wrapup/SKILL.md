...
## Execution model — 3 phases (recommended: mechanical part on Sonnet)

**Why recommended:** per the docs, `model:` frontmatter only overrides the session model "for the rest of the current turn" ([skills.md](https://code.claude.com/docs/en/skills.md)); `wrapup` is **multi-turn interactive** (retro gate, assumption-drift confirmation) → after every user gate it would fall back to the session model, so the mechanical bulk would run there again (: Opus throughout). Hence the skill carries **no** `model:` frontmatter; **recommended** instead is handing the mechanical git/gh plumbing to a **Sonnet subagent** (runs turn-independent on its own model). All user gates **and** the secret review stay in the **main thread** (session model).
...
**Dispatch contract (phase 1 → phase 2, only if your harness can do subagent dispatch with a model param):** once phase 1 is green (all gates answered, commit is local), dispatch **one** subagent — `Agent` tool, **`model: sonnet`** explicit (no `CLAUDE_CODE_SUBAGENT_MODEL` env pin set — otherwise it silently overrides the param; check `printenv | grep SUBAGENT` if unsure). Prompt hands over **all** values collected in phase 1:
...
**STOP-back rule (non-negotiable):** if the subagent hits **any** hard stop (push rejected · `pr-body-check` exit 1 · merge gate `CONFLICTING`/red check · merge not `MERGED` · `worktree remove`/`branch -d` refused · **any secret-grep hit** in the diff — already committed in phase 1) → **abort, force NOTHING, report back to the main thread** (reason + location). The subagent **does not resolve any security-judgment question itself** and does not call `AskUserQuestion` (it can't) — it reports, the main thread decides.
...
> The following steps are each tagged `[Phase N]` — that's the **recommended** assignment for subagent dispatch. The main thread executes phases 1 + 3; **everything tagged `[Phase 2]` runs in the Sonnet subagent, provided your harness supports it** — if it can't, the main thread executes those steps **itself, inline**, in the same order, without loosening the gates/rules.
<!-- mirror-xform:end -->
...
> **Phase split within Step 0:** 0a (commit + secret review) and the assumption-drift **confirmation** (0c.2) are `[Phase 1 · main thread]`; 0b (push) and the rest of 0c (PR/body/merge-gate) are `[Phase 2 · Sonnet subagent]`. The main thread gathers the inputs in 0a + 0c.2 (commit local, retro line, confirmed markers) and **then** dispatches 0b→5e.1 to the subagent.
<!-- mirror-xform:end -->
...
<!-- mirror-xform:start codex-wrapup-phase-labels -->
**Step 0b — unpushed → push.** `[Phase 2 · Sonnet subagent]` Feature branch → `pre-push` allowed (only `main` is blocked). Sets upstream idempotently:
<!-- mirror-xform:end -->
```bash
...
<!-- mirror-xform:start codex-wrapup-phase-labels -->
**Step 0c — ensure PR + merge gate.** `[Phase 2 · Sonnet subagent — EXCEPT 0c.2 drift confirmation = phase 1]` **Reuse** an existing PR (no abort!), otherwise create one; then check mergeability. The subagent gets the **finished** body text (retro line + confirmed markers) from phase 1 and only writes it — the marker confirmation (0c.2) was already done by the main thread **before** dispatch.
<!-- mirror-xform:end -->
```bash
...
- **Assumption-drift self-check (before merge —) — BEFORE the body check `[0c.2 · phase 1 · main thread]`:** the **confirmation** of the markers is done by the **main thread before dispatch** (user gate — a subagent can't get confirmation). **Writing** the confirmed markers into the body + the body check then happen in phase 2: write the markers into the body first (see below), THEN run the body-convention check, so it sees the **final** body (R2-F4). Source = the **build-time log** `ANNAHMEN.md` (worktree root, gitignored), kept live by wave-slice sessions (captured while context is fresh instead of from memory at session end; convention: CLAUDE.md cross-slice writeback + `tdd` checklist). **Q3-compliant: no code/diff scan, no heuristic — the log IS explicitly stated content, just captured earlier.** The log is the **floor, not the ceiling**: if drift shows up while landing that's **not** in the log, bring it in the same retro-style way (see fallback) instead of skipping it.
  ...
  - **Log missing/empty** (or a non-wave slice without a log) → **fallback, retro-style: I bring my own candidates first, I don't ask blank.** Like `/retro` (the user provides direction + approval, not the implementation detail — they usually don't know it, I built it): go through the assumptions **deliberately made or reversed** in the slice that might carry an **unbuilt** sibling issue (in program context this candidate pool reaches to not-yet-built future wave-stubs/leaves and the Program-PRD itself — see Step 5e.2's program-context target set), and present them as **named candidates** — each `- #<n>?: <assumption> → might carry <issue/contract>`. The user confirms / rejects / prioritizes; confirmed ones → markers as above, by hand. **Zero** candidates → **say so explicitly** ("no drift found — checked: <brief, what was touched>"), only then optionally the one-line safety net *"anything overlooked that carries an unbuilt issue?"*. The blank question is **never** a substitute for going through it yourself.
...
<!-- mirror-xform:start codex-wrapup-phase-labels -->
### Step 1 — merge the PR (= prod deploy) `[Phase 2 · Sonnet subagent]`
<!-- mirror-xform:end -->
...
<!-- mirror-xform:start codex-wrapup-phase-labels -->
### Step 2 — kill the worktree's dev server (BEFORE tearing down the worktree!) `[Phase 2 · Sonnet subagent]`
<!-- mirror-xform:end -->
...
<!-- mirror-xform:start codex-wrapup-phase-labels -->
### Step 4 — tear down the worktree (FROM the main tree) `[Phase 2 · Sonnet subagent]`
<!-- mirror-xform:end -->
...
<!-- mirror-xform:start codex-wrapup-phase-labels -->
### Step 5 — update main + delete the local branch `[Phase 2 · Sonnet subagent]`
<!-- mirror-xform:end -->
```bash
cd "$MAIN_TREE"
...
<!-- mirror-xform:start codex-wrapup-phase-labels -->
### Step 5b — verify issue close (catch auto-close misses) `[Phase 2 · Sonnet subagent]`
<!-- mirror-xform:end -->
...
<!-- mirror-xform:start codex-wrapup-phase-labels -->
### Step 5c — sweep orphaned merged branches (local stragglers) `[Phase 2 · Sonnet subagent]`
<!-- mirror-xform:end -->
...
<!-- mirror-xform:start codex-wrapup-phase-labels -->
### Step 5d — sweep orphaned merged branches (REMOTE stragglers) `[Phase 2 · Sonnet subagent]`
<!-- mirror-xform:end -->
...
<!-- mirror-xform:start codex-wrapup-phase-labels -->
1. **Anchor-tracker sync — `anchor-sync`** `[Phase 2 · Sonnet subagent]`. Instead of a manual tick: `python3 scripts/board-sync.py anchor-sync <anchor#> --dry-run` → review the diff → then write without `--dry-run`. This regenerates the **volatile columns of the slices table from the board** — **status** (`✅ #<PR>` merged · `🔄` open PR/in progress · `⬜` otherwise) and **branch** (actual PR `headRefName`) — **monotonically** (never flips an existing `✅`/`🔄` back if a sub-issue stays open as a `Part of`-gen-a/gen-b without a closing PR) and **drift-free idempotent** (a no-drift run reproduces the table byte-for-byte). **Stable plan columns** (slice/model/gate/blocked-by) stay **verbatim**; manual annotations like `✅ (gen-a)` survive. **Missing sub-issue rows** (mid-wave splits, the gen-b case) get appended as fresh rows — the output names them (`+N new sub-issue row(s)`); fill their **stable cells (slice/gate/model) by hand** afterward. **What `anchor-sync` does NOT manage:** the **gate symbol** (not board-tracked) → fix separately by hand if needed. The status column is thus **board-derived**, no longer hand-maintained; the native "sub-issues progress" rollup is the %-secondary view. Replaces the old manual tick + Step-6 reminder (— `anchor-sync` catches both a missing gen-b row and a stale branch). Script error / no slice table in the anchor → **stop + propose**.
<!-- mirror-xform:end -->
...
2. **Assumption propagation — PROPOSE + CONFIRM** (Q2) `[Phase 3 · main thread]`. Parse `annahme-drift` markers from the **subagent report** (resp. the PR body) (JSON). Per marker: draft a sibling edit (e.g. append to the target issue's `headings.vorBau` heading — <project> currently `## Vor Bau zu klären`) **+ re-stamp the sibling's `plan_revision`**; **show it to the user, get confirmation, only then write**. Silent writes are forbidden (Codex #15). Both are prose-led: the tick (1.) is a clear-cut one-line flip with no user gate, the substantive sibling edits are judgment → propose + confirm.
   - **Program-context target set.** When the anchor's own parent chain reaches a Program-PRD (native parent PRD → wave-stub → slice — the kit's `to-waves` top-down route), the legitimate target/candidate pool for propagation **widens** to include **not-yet-built future wave-stubs and their leaves under that PRD, and the Program-PRD issue itself** — same append-only mechanism, no new write path, just a wider target set for both the log-driven marker path and the fallback candidate walk below.
   - **Two-stage exception to the re-stamp default.** When the propagation target is the Program-PRD or one of its still-unbuilt wave-stubs, the append-only note does **NOT** re-stamp the target's `plan_revision` — that stays reserved for a genuinely **structural** wave-plan edit (rows changed/added/removed), which runs through the maintainer's escalation path instead (bump `plan_revision` + a `to-waves` delta re-run, which then renews every affected stub's `program-revision` marker in the same pass — see `to-waves`'s escalation path). Bumping `plan_revision` on a mere append-only drift note would falsely stale-block every already-published wave-stub; the two-stage split keeps drift-propagation cheap without corrupting revision coherence.

...
- **Type**: HITL / AFK
<!-- mirror-xform:start codex-escalation -->
- **Gate**: `—` (clean AFK build) · 🧭 Design Grill (`grill-with-docs-codex`, ADR-worthy) · 🔬 Verify Spike (read-only fact question) · 📐 Trade-off/Research (trade-off choice OR research below the grill threshold, read-only + documented trade-off) · 📝 Review Note (not a build slice). A gate slice (🧭/🔬/📐) is placed **before** its dependent build slice (gate-before-build) + blocks it.
<!-- mirror-xform:end -->

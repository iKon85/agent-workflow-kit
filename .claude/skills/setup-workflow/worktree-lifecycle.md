...
The exact kit-owned command is:

- PreToolUse on Edit/Write/MultiEdit:
  `python3 "$CLAUDE_PROJECT_DIR/.claude/hooks/enforce-worktree-cwd.py"`

It is the one lifecycle adapter the 2026-07 hook review kept (named incident:
repository issue 373). It judges the **write target** a structured payload
names, not the shell's working directory. A profile that still wires it under a
`Bash` matcher is not broken — the adapter simply has no opinion on a `Bash`
payload — but reconciliation moves it, so the guard reaches the writes it
protects. Wiring for the retired adapters (branch-context, branch-watch,
enforce-worktree, enforce-worktree-discipline, slice-handoff-hint) is consumer
data: setup never installs, rewires, or removes it.

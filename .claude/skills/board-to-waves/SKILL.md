...
description: Use when sweeping/grooming the GitHub backlog board to cluster open issues into thematic "Wellen" (waves). Triggers — (a) the user wants to go through / cluster the board ("lass uns das Board durchgehen", "was ist offen und passt zusammen", "clustern wir den Backlog", "welche Wellen stecken da drin"); (b) backlog operationalisation / Sichtung of many open issues (wie); (c) the user wants to spot candidate waves before planning. This skill READS all open issues, groups them by a Gate+Booster+Splitter heuristic, estimates size + risk per candidate, flags whether a dedicated `grill-with-docs` session is needed first, and — after the user confirms which candidates are real — creates cluster/Wave-less candidate STUB issues on the board with a To-Do checklist for maturing them. It STOPS at stubs: it does NOT write the PRD, plan slices, link sub-issues, or promote. Downstream: `to-prd` (matures the stub into a Draft-PRD) → `to-issues` (slices + promotes, sets type:cluster + Wave). Anti-trigger: maturing/slicing ONE already-chosen candidate (that is `to-prd`/`to-issues`), per-issue triage-role labelling (that is `triage`), or zooming into one code area for comprehension (that is `zoom-out`). board-to-waves is board-WIDE thematic affinity grouping, not per-issue and not code-reading.
---
